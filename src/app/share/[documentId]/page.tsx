...
const Page: React.FC<ParamsProps> = async ({ params: { documentId } }) => {
	...
	const { title, coverImage, iconImage, editorJson }: any = doc
	return (
		<div>
			<Header
				doc={doc}
			...
			<ScrollArea
				...
				<main className='flex flex-col h-[inherit]'>
					...
					<section className='flex flex-col flex-1 w-full'>
						<div
							...
							{iconImage && (
								<div
									...
									<Image
										className={cn(
											'w-[125px] h-[125px] object-cover bg-accent'
										)}
										alt='icons image'
						...
						{editorJson ? (
							<EditorPreviewer editorJson={editorJson} />
						) : null}
					</section>
				</main>
			</ScrollArea>
...
export async function generateMetadata(
	...
	// fetch data
	const document: any = await findDoc(documentId)
